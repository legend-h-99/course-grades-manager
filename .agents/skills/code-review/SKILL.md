...
Data handling: the CLI sends code diffs to the CodeRabbit API for analysis. Before running a review, confirm the complete selected diff does not contain secrets or credentials. Match the secret scan to the review scope: staged and unstaged changes for the default/all scope, committed changes for `-t committed`, uncommitted changes for `-t uncommitted`, and the selected comparison for `--base` or `--base-commit`. Use the narrowest token scope when authenticating (`coderabbit auth login`).
